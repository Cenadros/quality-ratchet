...
def make() -> Baseline:
    return new_baseline(CURRENT, VERSIONS, "abc1234", DEFAULT_WEIGHTS, CFG_HASH)
...
def test_ratchet_only_improves_without_force():
    ...
    new, changed = ratchet(b, now, VERSIONS, "def5678", DEFAULT_WEIGHTS, CFG_HASH)
...
def test_ratchet_no_change_keeps_commit():
    b = make()
    new, changed = ratchet(b, dict(CURRENT), VERSIONS, "def5678", DEFAULT_WEIGHTS, CFG_HASH)
...
def test_ratchet_refuses_config_change_without_force():
    b = make()
    now = {**CURRENT, "lint_warnings": 90}  # an improvement, to prove the hash gate fires first
    with pytest.raises(ConfigError, match=r"config changed since baseline \(hash abc123def456 → zzz999\)"):
        ratchet(b, now, VERSIONS, "def5678", DEFAULT_WEIGHTS, "zzz999")


def test_ratchet_accepts_config_change_with_force():
    b = make()
    now = {**CURRENT, "max_ccn": 31}
    new, changed = ratchet(b, now, VERSIONS, "def5678", DEFAULT_WEIGHTS, "zzz999", force=True, reason="tightened exclude")
    assert changed == ["max_ccn"]
    assert new.config_hash == "zzz999"
